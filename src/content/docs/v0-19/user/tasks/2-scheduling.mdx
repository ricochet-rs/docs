...
This includes R, Julia, and Python scripts, R Markdown, and Quarto documents.
...
Tasks can be scheduled using the ricochet UI or the REST API.

### Using the UI

You can schedule a task from the ricochet UI by navigating to the "Schedule" settings in the runtime configuration page (e.g., `https://try.ricochet.rs/tasks/{item-id}/runtime`).

### Using the REST API

```shell
curl -X PATCH \
  'https://try.ricochet.rs/api/v0/content/01JT1AAJY0EVDE4C0277XX148R/schedule' \
...
`L` is used to denote the last day of the week or month.
...
The `#` modifier can be used to specify which occurence of a particular day in a month.
...
The `W` modifier can be used to specify the closes weekday to a day of the month.
