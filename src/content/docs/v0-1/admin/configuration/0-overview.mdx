...
[auth.oidc]
issuer_url = "https://auth.ricochet.rs"
...
ricochet requires persistent storage for its database, deployments, encryption keys and more which we call `RICOCHET_HOME`.
...
ricochet uses the below precedence for configuration values. Enviroment variables are prioritized over the `ricochet-config.toml` which is prioritized over default values.
...
<Aside>
We recommend using `ricochet-config.toml` for configuration whenever possible and environment variables for overrides or dynamic settings when starting ricochet.
</Aside>
...
<LinkCard
  ...
  href="/v0-1/admin/configuration/execution-engine/"
  description="Use Kubernetes, Docker, or Podman as an execution engine."
/>
