...
When using the standalone binary with `--config`, the `home` field is required in your `ricochet-config.toml`.
Always set `home` to an absolute path to ensure consistent behavior regardless of the directory you launch from.
