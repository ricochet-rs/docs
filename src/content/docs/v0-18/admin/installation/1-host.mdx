...
The command exits without making changes on systems that do not use AppArmor or do not restrict unprivileged user namespaces.
...
Move-Item .\ricochet-server.exe "$env:LOCALAPPDATA\Programs\Ricochet\ricochet.exe" -Force
```
...
```powershell
ricochet start --config C:\path\to\ricochet-config.toml
```

Windows does not support `ricochet start --daemon` or `ricochet stop`.
