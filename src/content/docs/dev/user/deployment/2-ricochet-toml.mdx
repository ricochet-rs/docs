...
The optional `[package]` section lets you override the default package repository sources for your content item.
This is useful when your packages are hosted on a private registry or a specific mirror.

<Aside type="note">
Item-level package overrides only take effect if the server administrator has enabled `allow_overrides` in the server configuration.
