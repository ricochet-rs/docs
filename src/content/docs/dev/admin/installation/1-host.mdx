...
The `--config` flag specifies the path to your `ricochet-config.toml`. If omitted, Ricochet looks for `ricochet-config.toml` in the current working directory.
...
These files are separate from any file-based logging configured via tracing settings.
