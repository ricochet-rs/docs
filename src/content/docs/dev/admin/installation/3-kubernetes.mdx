...
When running in Kubernetes, store these in a Kubernetes Secret so multiple server replicas can share one key.
...
A relative path resolves under `RICOCHET_HOME`, so give the mounted Secret an absolute path.
By default, Ricochet creates these credentials once during initial startup.
