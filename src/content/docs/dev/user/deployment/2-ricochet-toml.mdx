...
| `content_type` | Specifies the content type to be deployed. See [Content Items Overview](/dev/user/content-items/1-content-items-overview) for the full list. Valid values: `"r"`, `"rmd"`, `"plumber"`, `"ambiorix"`, `"shiny"` / `"shiny-r"`, `"rmd-shiny"`, `"r-service"`, `"serverless-r"`, `"quarto-r"`, `"quarto-r-shiny"`, `"julia"`, `"julia-service"`, `"quarto-jl"`, `"python"`, `"python-service"`, `"quarto-py"`, `"fast-api"`, `"flask"`, `"streamlit"`, `"dash"`, `"shiny-py"`. |
...
See the [admin package repositories guide](/dev/admin/configuration/package-repositories/) for details on precedence rules.
</Aside>
