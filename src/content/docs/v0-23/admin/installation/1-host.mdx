...
See [File Logging](/v0-23/admin/configuration/file-logging/) for configuration options.
...
Keep the `ricochet-server` name so that the server does not shadow the [`ricochet` CLI](/v0-23/user/deployment/1-ricochet-cli/), which is commonly installed on the same machine.
...
Keep the `ricochet-server.exe` name so that the server does not shadow the [`ricochet` CLI](/v0-23/user/deployment/1-ricochet-cli/), which is commonly installed on the same machine.
...
See the [storage configuration](/v0-23/admin/configuration/0-overview/#storage) for more details.
