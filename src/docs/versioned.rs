...
pub fn get_doc_pages_for_version(version: &Version) -> &'static [DocPage] {
    match version.path {
        "v0.1" => &DOC_PAGES_V0_1,
        "dev" => &DOC_PAGES_DEV,
...
pub fn get_doc_sections_for_version(
    version: &Version,
) -> BTreeMap<DocSection, Vec<&'static DocPage>> {
    let mut map = BTreeMap::new();
    let pages = get_doc_pages_for_version(version);

    let mut quick = Vec::new();
    let mut content = Vec::new();
    let mut admin = Vec::new();

    for doc in pages.iter() {
        match &doc.section {
