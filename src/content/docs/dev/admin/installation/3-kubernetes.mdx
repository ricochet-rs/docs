...
Ricochet encrypts environment variables and git credentials with a keypair stored in `.ricochet.json` on the home volume.
Store it in a Kubernetes Secret instead so multiple server replicas can share one key.
...
`RICOCHET_SECRETS` is the path to the directory that holds `.ricochet.json`.
It sets the `secrets` configuration field, which defaults to `RICOCHET_HOME`.
A relative path resolves under `RICOCHET_HOME`, so give a mounted Secret an absolute path.

By default, ricochet creates credentials at startup.
It is preferred to set the path so that the credentials are created by ricochet in the correct location.
Do not run `ricochet secrets-init` after starting a new ricochet server.

<Aside type="caution">
  Do not run `ricochet secrets-init` for an instance that already holds data.
  Replacing the keypair makes existing encrypted values unrecoverable.
</Aside>
