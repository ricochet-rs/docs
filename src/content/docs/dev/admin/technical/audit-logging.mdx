...
Records are stored in the database, in the `audit_logs` table, and written in batches every 200ms or every 256 events, whichever comes first.

Read them on the **Audit Log** page, or [export](#exporting) them.

:::note
Earlier versions also wrote a JSON Lines file under `RICOCHET_HOME/logs/audit/`.
That file is no longer written or read, and any archives already on disk are left alone for you to remove.
:::

## Retention

Records are kept for 365 days by default.
...
A server admin can download the whole log from the **Audit Log** page as CSV or JSON.
