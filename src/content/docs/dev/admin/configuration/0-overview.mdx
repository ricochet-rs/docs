...
<Aside type="tip">
  When using the standalone binary, it is recommended to set `home` to an absolute path in your `ricochet-config.toml`.
  This ensures consistent behavior regardless of the directory you launch from.
</Aside>

Alternatively, use the `RICOCHET_HOME` environment variable to specify where ricochet should use the home directory.

To keep environment variables and credentials secure, ricochet uses a hybrid encryption mechanism.
The keys are stored at `RICOCHET_HOME/.ricochet.json` and `RICOCHET_HOME/.ricochet.pub`.
To specify an alternative location to set or source these variables, use the `secrets` field in the `ricochet-config.toml`.
This can be useful in particular when using the Kubernetes backend.
See [Encryption keys](/dev/admin/installation/3-kubernetes/#encryption-keys) for more details.
...
<LinkCard
  ...
  href="/dev/admin/configuration/1-authentication/"
  description="Configure OIDC authentication with your identity provider."
/>

<LinkCard
  title="User Roles"
  href="/dev/admin/configuration/2-user-roles/"
  description="Configure default user roles and understand ricochet's permission system."
/>
<LinkCard
  title="Package Repositories"
  href="/dev/admin/configuration/package-repositories/"
  description="Override default package sources for R and Python content items."
/>
<LinkCard
  title="Execution Backend"
  href="/dev/admin/configuration/execution-backend/"
