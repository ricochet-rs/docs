...
The `secrets` configuration reads `.ricochet.json` and `.ricochet.pub` from a directory of your choice instead of `RICOCHET_HOME`, resolving a relative path under it.
Use it to keep the encryption keypair in a Kubernetes Secret rather than on the home volume.
