...
View audit log summaries on the **Audit Log** page, or [export the logs](#exporting).
...
| `authority` | Whether a global admin override was used (see [Authority](#authority)) |

## Authority

The `authority` field records direct permissions as `direct` and admin override privileges as `global_admin_override`.
