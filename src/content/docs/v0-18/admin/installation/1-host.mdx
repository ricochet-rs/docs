...
`ricochet-server` is distributed as native Linux packages and standalone binaries.
This guide covers manual installation on a single host.
...
| AlmaLinux / RHEL / EL | x86_64, aarch64 | `.rpm`      |
| Other Linux           | x86_64, aarch64 | `.tar.gz`   |
...
<Tabs>
  ...
  <TabItem label="RHEL / AlmaLinux">
    ...
    VERSION=$(curl -s https://ricochet.rs/releases.json | grep -o '"latest": *"[^"]*"' | grep -o 'v[0-9.]*')
    ARCH=$(uname -m)
    curl -fLO https://s3.eu-central-003.backblazeb2.com/ricochet-server/${VERSION#v}/ricochet-server-${VERSION#v}-1.el10.${ARCH}.rpm
    sudo dnf install ./ricochet-server-${VERSION#v}-1.el10.${ARCH}.rpm
    ```
  </TabItem>
  <TabItem label="Standalone binary">
    ...
    # Extract and install as ricochet
    tar -xzf ricochet-${VERSION#v}-*.tar.gz
    chmod +x ricochet-server
    sudo mv ricochet-server /usr/local/bin/ricochet
    ```
...
The `.deb` and `.rpm` packages configure these prerequisites automatically.
Complete these steps only when installing the standalone binary.
...
<Tabs>
  ...
  <TabItem label="RHEL / AlmaLinux">
    ```sh
    sudo dnf install shadow-utils-subid
    ```
  </TabItem>
</Tabs>

Grant the account that runs ricochet at least 65,536 subordinate user and group IDs.
The following example configures `root`, which is the account used by the packaged systemd service.
Choose an unused range if `100000-165535` is already allocated in `/etc/subuid` or `/etc/subgid`:
...
See [File Logging](/v0-18/admin/configuration/file-logging/) for configuration options.
...
<Tabs>
  ...
  <TabItem label="RHEL / AlmaLinux">
    ...
    <Aside type="note">
      Use `dnf install` instead of `dnf upgrade` because it handles fresh installs, upgrades, and downgrades.
    </Aside>
  </TabItem>
</Tabs>
...
`ricochet-server` is distributed as a standalone binary for Apple Silicon Macs.
...
Keep the `ricochet-server` name so that the server does not shadow the [`ricochet` CLI](/v0-18/user/deployment/1-ricochet-cli/), which is commonly installed on the same machine.

Create a configuration file as described in [Configure](#configure), set `home` to an absolute writable path, and start the server in the foreground:

```sh
ricochet-server start --config /path/to/ricochet-config.toml
```

The macOS binary does not include a service definition or vendored language dependencies.
Install `uv`, `juliaup`, and `renv` separately when you need the corresponding runtimes.

## Windows

`ricochet-server` is distributed as a standalone executable for x86_64 and arm64 Windows.
Run these commands in PowerShell, replacing `x86_64` with `aarch64` on Windows arm64:
...
Keep the `ricochet-server.exe` name so that the server does not shadow the [`ricochet` CLI](/v0-18/user/deployment/1-ricochet-cli/), which is commonly installed on the same machine.

Add `%LOCALAPPDATA%\Programs\Ricochet` to your user `PATH`, then create a configuration file as described in [Configure](#configure).
Set `home` to an absolute writable Windows path.
...
Windows does not support `ricochet-server start --daemon` or `ricochet-server stop`.
Stop the foreground server with Ctrl+C, or configure it with a Windows service manager.
The Windows binary does not include vendored language dependencies.
...
See the [storage configuration](/v0-18/admin/configuration/0-overview/#storage) for more details.
