...
<Tabs>
  ...
  <TabItem label="Standalone binary">
    We also provide statically linked binaries which are available for x86_64 and aarch64:
    ...
    <Aside type="note">
      The standalone binary does not include systemd service configuration. You will need to manage the process manually or create your own service file.
    </Aside>
  </TabItem>
</Tabs>
...
When installing on a host, the R, Python, and Julia interpreters must be installed directly on the machine.
...
See the [storage configuration](/v0-1/admin/configuration/0-overview/#storage) for more details.

### Python

Ricochet **requires** Python to be installed via [uv](https://docs.astral.sh/uv/). Only uv-managed Python installations are supported.
...
We recommend to use [rig](https://github.com/r-lib/rig) to install R:
...
Ricochet **requires** Julia to be installed via [juliaup](https://github.com/JuliaLang/juliaup). Only juliaup-managed Julia installations are supported.
