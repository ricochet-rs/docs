...
<Aside>
User roles are separate from content-level collaborator roles. A developer might be an "owner" of one item and a "contributor" on another item.
</Aside>
...
**Initial Login**: When users authenticate for the first time via OIDC, they are automatically assigned the default role configured in your settings (see [Configuration](#configuration) above).
