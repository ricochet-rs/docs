...
<Aside type="note">
  ...
  See [authentication configuration](/dev/admin/configuration/1-authentication) for details.
</Aside>
...
ricochet_url <- Sys.getenv("RICOCHET_HOST")
...
ricochet_url = os.environ["RICOCHET_HOST"]
...
# audience verifies aud, exp is checked automatically
claims = jwt.decode(
    token,
    signing_key.key,
...
The examples below show the full pattern for each.
