...
Install the R package via:

```r
# note this is not on CRAN
# email contact@ricochet.rs for assistance
install.packages("ricochet")
```
...
Each item that is deployed to **`ricochet`** requires an [`renv.lock`](https://rstudio.github.io/renv/) and a [`_ricochet.toml`](/ricochet-toml) file. The `renv.lock` file is used to track dependencies for your deployment item. The `_ricochet.toml` defines the type of deployment as well as associated metadata.
