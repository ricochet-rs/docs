...
See [File Logging](/dev/admin/configuration/file-logging/) for configuration options.
...
Keep the `ricochet-server` name so that the server does not shadow the [`ricochet` CLI](/dev/user/deployment/1-ricochet-cli/), which is commonly installed on the same machine.
...
Keep the `ricochet-server.exe` name so that the server does not shadow the [`ricochet` CLI](/dev/user/deployment/1-ricochet-cli/), which is commonly installed on the same machine.
